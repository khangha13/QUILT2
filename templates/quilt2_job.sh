...
if [ "$#" -lt 17 ]; then
    log_error "Usage: quilt2_job.sh <WORK_DIR> <CHUNK_MANIFEST> <REFERENCE_PANEL_DIR> <GENETIC_MAP> <GENETIC_MAP_IS_DIR> <QUILT2_PREP_SCRIPT> <QUILT2_RUN_SCRIPT> <BAMLIST> <NGEN> <REMOVE_MISSING> <MIN_PHASED_RATE> <PREP_ONLY> <IMPUTE_ONLY> <OUTPUT_DIR> <PANEL_OUT_DIR> <RDATA_DIR> <TMP_DIR> [BCFTOOLS_MODULE] [QUILT2_CONDA_ENV]"
    exit 1
fi
...
export REMOVE_MISSING MIN_PHASED_RATE PANEL_OUT_DIR
